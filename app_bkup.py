...
class Hydra(toga.App):
    ...
    def startup(self):
        ...
        self.file_input = toga.TextInput(style=Pack(flex=1))
        file_box = toga.Box(style=Pack(direction=ROW, padding=5)) 
        file_box.add(file_label)
        file_box.add(self.file_input)
        ...
        term_button = toga.Button(
            'Add term, press Fetch when done adding terms',
    ...
    def search_geneid(self,terms, browser): 
        ...
        try:
            self.keywordElem = self.browser.find_element_by_xpath(self.keyword_box)
        ...
        for self.term in self.terms: 
            ...
            try:
                self.buttonElm = browser.find_element_by_xpath(self.button).click()
                print ('Found buttonElm with that xpath')
            except:
                print('Was not able to find buttonPress.')

            #If, for some reason, the links can't be found, it could be due to the website 
            #loading slowly. Time.sleep() gives the browser time to complete loading. 
            #time.sleep(3) 
    ...
    def gb_link(self,geneid, browser): 
        ...
        try:
            link_to_genome = self.browser.find_element_by_link_text('View Gene in Genome Browser').click()
            print ('Found View_gene_elem with that xpath')
        except:
            print('Was not able to find View_gene_elem')
        
        # After clicking the button, store the window handle variable the view_in_broswer page
        #Then wait for the jbrowse page to load and store that window handle varieable 
        view_in_browser_page = self.browser.window_handles[0]
        print ('View in browser page time sleep 5')
        time.sleep(5)
        jbrowser_page = self.browser.window_handles[1] 
